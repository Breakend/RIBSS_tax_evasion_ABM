...
while(t < final.year || (run.till.equilibrium == TRUE && is.in.equilibrium != TRUE)) { 
  ...
  if(t==200){pop.data$tax.rate <- pop.data$tax.rate}
  ...
  if(is.null(gov.dyn)) { #If gov.dyn is NULL it implies no scale was calculated before
    ...
    gov.dyn$US.Debt <- US.Debt
     ...
     inflated.indicators <- get.Debt.and.GDP.with.growth(year=t-start.year,
     ...
     gov.dyn <- cbind.data.frame(gov.dyn,inflated.indicators)
    ...
    initial.deficit <- gov.dyn$US.deficit[1]
    
    US.marginal.Debt <- last(gov.dyn$US.marginal.Debt)
    US.Debt <- last(gov.dyn$US.Debt)
    previous.year.inflated.Debt <- last(gov.dyn$inflated.Debt)
...
last.t.years <- 30
plots <- create.plots(sim.data, track.id, aggregated.dyn, config, g.info, 
                      last.t.years = last.t.years)

display.or.save.plots(plots, figures.dir, print.plots = TRUE, save.plots = TRUE,
                      extra.label="March14-2018")

plot(aggregated.dyn$tax.gap)
print(plots$report.plot)
print(table(non.compliant.non.penalized))

gov.plots<-gov.dyn.plot(gov.dyn, small = 14,big = 18,
                        font.reduction.factor=0.75,
                        starting.year=2016)

display.or.save.plots(gov.plots, figures.dir, print.plots = TRUE, save.plots = TRUE,
                      extra.label="March14-2018")
