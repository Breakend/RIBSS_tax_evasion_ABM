my.table.sd2 <- function(x,var,var1,var2,weight,order.col=NA,order.row=NA){
  require(Hmisc)
  if (!is.null(var2)){
    a <-  split(x,list(x[,var1],x[,var2]),drop=TRUE)
    s <-  sapply(a,FUN=function(x){sqrt(wtd.var(x[,var],x[,weight],normwt=TRUE,na.rm=TRUE))})
    ...
    if (!identical(order.row,NA)) tab <- tab[order.row,]
        ...
        tab <-  sapply(s,FUN=function(x){sqrt(wtd.var(x[,var],x[,weight],normwt=TRUE,na.rm=TRUE))})
  }
  return(tab)
}
