...
# create a square term for the income variable (ego_income_new_sqrd)
ego_income_new_sqrd <- ego_income_new^2

# create an income variable in terms of thousand dollars
ego_income_inthousand <- ego_income_new/1000
ego_income_inthousand_sqrd <- ego_income_inthousand^2

# put ego_income_new and ego_income_new_sqrd into the tim data frame
tim[, "ego_income_new"] <- ego_income_new
tim[,"ego_income_new_sqrd"] <- ego_income_new_sqrd
tim[,"ego_income_inthousand"] <- ego_income_inthousand
tim[,"ego_income_inthousand_sqrd"] <- ego_income_inthousand_sqrd

# generate binary income variables using income1
tim$bracket1 <- tim$income1
tim$bracket1[tim$income1 != 1] <- 0

tim$bracket2 <- tim$income1
tim$bracket2[tim$income1 != 2] <- 0
tim$bracket2[tim$income1 == 2] <- 1

tim$bracket3 <- tim$income1
tim$bracket3[tim$income1 != 3] <- 0
tim$bracket3[tim$income1 == 3] <- 1

tim$bracket4 <- tim$income1
tim$bracket4[tim$income1 != 4] <- 0
tim$bracket4[tim$income1 == 4] <- 1

tim$bracket5 <- tim$income1
tim$bracket5[tim$income1 != 5] <- 0
tim$bracket5[tim$income1 == 5] <- 1

tim$bracket6 <- tim$income1
tim$bracket6[tim$income1 != 6] <- 0
tim$bracket6[tim$income1 == 6] <- 1

tim$bracket7 <- tim$income1
tim$bracket7[tim$income1 != 7] <- 0
tim$bracket7[tim$income1 == 7] <- 1

tim$bracket8 <- tim$income1
tim$bracket8[tim$income1 != 8] <- 0
tim$bracket8[tim$income1 == 8] <- 1

tim$bracket9 <- tim$income1
tim$bracket9[tim$income1 != 9] <- 0
tim$bracket9[tim$income1 == 9] <- 1

tim$bracket10 <- tim$income1
tim$bracket10[tim$income1 != 10] <- 0
tim$bracket10[tim$income1 == 10] <- 1

tim$bracket11 <- tim$income1
tim$bracket11[tim$income1 != 11] <- 0
tim$bracket11[tim$income1 == 11] <- 1

tim$bracket12 <- tim$income1
tim$bracket12[tim$income1 != 12] <- 0
tim$bracket12[tim$income1 == 12] <- 1

tim$bracket13 <- tim$income1
tim$bracket13[tim$income1 != 13] <- 0
tim$bracket13[tim$income1 == 13] <- 1

tim$bracket14 <- tim$income1
tim$bracket14[tim$income1 != 14] <- 0
tim$bracket14[tim$income1 == 14] <- 1
...
lrfit0 <- glm(formula = selfemployed ~ gender + age + ego_income_new, data=tim, family = binomial)
...
lrfit1 <- glm(formula = selfemployed ~ gender + age + ego_income_new + alter_ed_income_average, data= tim, family = binomial)
...
lrfit2 <- glm(formula = selfemployed ~ gender + age + ego_income_new + alter_se_proportion + alter_ed_income_average, data= tim, family = binomial)

summary(lrfit2)

# regression without alter variables but with a square term for income variable
# use glm to do a logistic regression where selfemployed depends on gender, age, ego_income_new and ego_income_new_sqrd
lrfit0a <- glm(formula = selfemployed ~ gender + age + ego_income_new + ego_income_new_sqrd, data=tim, family = binomial)

summary(lrfit0a)


# regression with 1 alter variable and with a square term for income variable
# use glm to do a logistic regression where selfemployed depends on gender, age, ego_income_new, alter_ed_income_average and ego_income_new_sqrd
lrfit1a <- glm(formula = selfemployed ~ gender + age + ego_income_new + ego_income_new_sqrd + alter_ed_income_average, data= tim, family = binomial)

summary(lrfit1a)

# regression with alter variables
# use glm to do a logistic regression where selfemployed depends on gender, age, ego_income_new, alter_se_proportion, alter_ed_income_average and ego_income_new_sqrd
lrfit2a <- glm(formula = selfemployed ~ gender + age + ego_income_new + ego_income_new_sqrd + alter_se_proportion + alter_ed_income_average, data= tim, family = binomial)

summary(lrfit2a)


# models with ego_income_inthousand
# a model but with ego_income_inthousand
lrfit0b <- glm(formula = selfemployed ~ gender + age + ego_income_inthousand, data=tim, family = binomial)

summary(lrfit0b)

# a model but with square term of ego_income_inthousand
lrfit1b <- glm(formula = selfemployed ~ gender + age + ego_income_inthousand + ego_income_inthousand_sqrd, data=tim, family = binomial)
...
print(reg.out,  
      file=paste(survey.output.dir ,"Regression_Self_Employed_GA",".tex",sep=""))
...
# regression with alter variables
# use glm to do a logistic regression where selfemployed depends on gender, age, income brackets, alter_se_proportion
lrfit3 <- glm(formula = selfemployed ~ gender + age  + alter_se_proportion + bracket1 + bracket2 + bracket3 + bracket4 + bracket5 + bracket6 +
                bracket7 + bracket8 + bracket9 + bracket10 + bracket11 + bracket13 + bracket12, data= tim, family = binomial)
#bracket14 is dropped from the model to make it as a reference point
summary(lrfit3)

# use glm to do a logistic regression where selfemployed depends on gender, age, income brackets, alter_se_proportion
lrfit4 <- glm(formula = selfemployed ~ gender + age  + alter_se_proportion + bracket1 + bracket2 + bracket3 + bracket4 + bracket5 + bracket6 +
                bracket7 + bracket8 + bracket9 + bracket10 + bracket11 + bracket13 + bracket14, data= tim, family = binomial)
#bracket12 is dropped from the model to make it as a reference point
summary(lrfit4)

# use glm to do a logistic regression where selfemployed depends on gender, age, income brackets, alter_se_proportion
lrfit5 <- glm(formula = selfemployed ~ gender + age  + alter_se_proportion + bracket1 + bracket2 + bracket3 + bracket4 + bracket5 + bracket6 +
                bracket7 + bracket8 + bracket9 + bracket12 + bracket11 + bracket13 + bracket14, data= tim, family = binomial)
#bracket10 is dropped from the model to make it as a reference point
summary(lrfit5)

reg.out2 <- texreg(list(lrfit1,lrfit2),
...
print(reg.out2,  
      file=paste(survey.output.dir ,"Regression_Self_Employed_consolidated_GA",".tex",sep=""))

reg.out3 <- texreg(list(lrfit1,lrfit2, lrfit3, lrfit4, lrfit5),
                   caption="Consolidated Regression Models predicting self-employment status (including the models with income brackets)",
                   dcolumn=FALSE,
                   override.se=list(summary(lrfit1)$coef[,2],
                                    summary(lrfit2)$coef[,2],
                                    summary(lrfit3)$coef[,2],
                                    summary(lrfit4)$coef[,2],
                                    summary(lrfit5)$coef[,2]),
                   override.pval=list(summary(lrfit1)$coef[,4],
                                      summary(lrfit2)$coef[,4],
                                      summary(lrfit3)$coef[,4],
                                      summary(lrfit4)$coef[,4],
                                      summary(lrfit5)$coef[,4]))


print(reg.out3,  
      file=paste(survey.output.dir ,"Regression_Self_Employed_consolidated_With_Income_Brackets",".tex",sep=""))


se.reg.models <- list(se.reg1=lrfit1,se.reg2=lrfit2)
saveRDS(se.reg.models,file=paste(survey.data.dir,"se.reg.models_GA",".Rdata",sep=""))
