...
exectution.start.time<-Sys.time()
...
detection.eff<- 0.8 ### 80% detection and 100% discovery: all amount undereported is found if detected.
penalty.rate <- 0.25
K <- 3

#############################################################
###        Tax Complexity Parameters.           
#############################################################

tax.complexity <- 0

#############################################################
###        Tax Gap Media Parameters            
#############################################################

tax.gap <- 0

if(media.effect){
  beta.morale.media <- 0.5
}else{
  beta.morale.media <- 0.0
}
media.feedback <- 0
media.mid.effect <- 0.2973547 ### at 35% tax gap media attention is at 50% of the saturation point. 
media.steepness <- 1.791555    ### steepness of the media effect: its invers is a log normal stand. dev. 
media.stochastic.offset <- 0.3683569  ## offset modeling stochastic episodes due to also tax avoidance reporting. 
...
beta.morale.network <- 0.1
beta.audit  <- beta.morale.network
...
for(t in 1:final.year){
  ...
  if (t <=100) {tax.rate <- 0.3}
  if (t >100) {tax.rate <- 0.15}
  print(t)
  ...
  if(any(compliant.or.penalized)){
    Delta.Per[compliant.or.penalized] <- 
    ...
    if(tax.refund.effect){
      ...
      Delta.Per[tmp.pop] <- (1-return.weight)*Delta.Per[tmp.pop]+ return.weight*1
      tmp.pop <- (!refund.return & compliant.or.penalized) 
      Delta.Per[tmp.pop] <- (1-return.weight)*Delta.Per[tmp.pop]
    }
  ...
  if(any(non.compliant.non.penalized)){
    Delta.Per[non.compliant.non.penalized] <- 0 
    ...
    if(mild.tendency.to.full.evasion){
      ...
      if(any(partial)){
      Delta.Per[partial] <- fairness.fn(c1.tilde[partial],c1,c2,tax.rate)
      }
    }
  }
  ...
  #############################################################
  if(network.interactions){
    network.values <- get.network.interactions(Delta.Per,tax.ids, beta.morale.network,nn.int,
                                               prop.of.those.int)
  ...
  Delta.Morale <- (1-effective.beta.morale.network)*Delta.Per + 
  ...
  V.new <- s.discount*V.past+Delta.Morale
  ...
  initiation.prob <- 1-propensity.prop.income.report[suseptibles.to.underreport.pop]
  ...
  w[suseptibles.to.underreport.pop] <- 1-initiation.prob
  ...
  track.dyn[[t]] <- data.frame(t=t,
                               ...
                               Delta.Morale=round(Delta.Morale,2),
                               V.new=round(V.new,2),
                               w=round(w,2),
                               compliant=compliant,
                               report=report,
...
out <- rbind(c1=round(100*c1,0),
             ...
             Delta.Per= round(100*Delta.Per ,0),
...
exectution.end.time<-Sys.time()

print(paste("Exceution time =", exectution.end.time-exectution.end.time))
...
dat$compliant.group<- "compliantin the last 40 years"
dat$compliant.group[dat$years.since.last.compliant>generation.half.life] <- "never-compliant in last 40 years"
dat$compliant.group[dat$years.since.last.compliant<2*morale.half.life ] <- "compliant in last 4 years"
dat$audit.group<- "audited but not in last 4 years"
dat$audit.group[dat$years.since.last.audit>generation.half.life] <- "never audited in last 40 years"
dat$audit.group[dat$years.since.last.audit<2*morale.half.life ] <- "recently audited"
dat$pen.group<- "penalized but not in last 4 years"
dat$pen.group[dat$years.since.last.penalty>generation.half.life] <- "never penalized in last 40 years"
dat$pen.group[dat$years.since.last.penalty<2*morale.half.life ] <- "recently penalized"
...
c1.plot <- ggplot(dat) +
  geom_histogram(aes(x=c1,y=..density..),alpha=0.55, fill="pink",color="pink",bins = 50)+
  geom_histogram(aes(x=c1.tilde,y=..density..),alpha=0.55, fill="lightblue",color="blue",bins= 50)+
...
w.plot <- ggplot(dat) +
  geom_histogram(aes(x=w,fill = compliant.group),alpha=1, color="black",bins=50) +
  theme_bw() +
...
report.plot <- ggplot(dat) +
  geom_histogram(aes(x=report,fill = compliant.group ),alpha=1,  color="black",bins=50) +
  theme_bw() +
...
per.audit.rate.plot <- ggplot(dat) +
  geom_histogram(aes(x=per.audit.rate,fill=audit.group),alpha=1, color="black",bins=100) +
  theme_bw() +
...
per.penalty.rate.plot <- ggplot(dat) +
  geom_histogram(aes(x=per.penalty.rate, fill =pen.group),alpha=1, color="black",bins = 100) +
  theme_bw() +
...
years.since.last.compliant.plot <- ggplot(dat) +
  geom_histogram(aes(x=years.since.last.compliant,fill =audit.group), color="black",bins= 50) +
  theme_bw() +
...
Delta.Network.vs.Per.plot <- ggplot(dat) +
  geom_histogram(aes(x=Delta.Network,fill =as.factor(round(Delta.Per/100,1))),bins = 10) +
  theme_bw() +
...
Delta.Per.vs.Network.plot <- ggplot(dat) +
  geom_histogram(aes(x=Delta.Per,fill =as.factor(round(Delta.Network/100,1))),bins = 10) +
  theme_bw() +
  guides(fill = guide_legend(title="Level of Delta.Network"))+
  xlab("Delta.Per") +
  ylab("count")+
  ggtitle(paste("Histogram Delta.Per for different Delta.Network",sep="")) 
...
sample.report.trajectory.plot<- ggplot(track.id.focus.ids) +
  ...
  xlab("year") +
  ylab("report")+
  ggtitle(paste("Percentage of income reported for various tax payers of interest",sep=""))+
...
tmp <- track.id.focus.ids[track.id.focus.ids$tax.ids%in%707,]
tmp2 <- track.id.focus.ids.penalized[track.id.focus.ids.penalized$tax.ids%in%707,]
single.report.trajectory.plot1<- ggplot(tmp) +
  geom_line(aes(x=t, y=report, group= tax.ids),color="#00BADE",size=2)+
  geom_vline(data =tmp2, aes(xintercept = t, colour= type, linetype=type),size=2) +
  scale_color_manual(values=myColors)+
  xlab("year") +
  ylab("report")+
  ggtitle(paste("Percentage of income reported for tax payer 707",sep=""))+
  theme(axis.text.x=element_text(size=16 ), 
        axis.text.y=element_text(size=16 ) ,
        title = element_text( size=18 ) ,
        strip.text=element_text( size=14 ) ,
...
tmp <- melt.aggregated.dyn[melt.aggregated.dyn$agg_variable%in%"tax.gap",]
tmp$agg_value <- tmp$agg_value*100
aggregated.dyn.plot2 <- ggplot(tmp)+
  geom_line(aes(x=t, y=agg_value, group= agg_variable, colour = agg_variable),size=1.5)+
  #facet_wrap(~ agg_variable)+
  ggtitle(paste("Tax Gap Dynamics",sep=""))+
  xlab("year")+
  ylab("percentage")+
  theme(axis.text.x=element_text(size=16 ), 
        axis.text.y=element_text(size=16 ) ,
...
t<- 100
frame <- track.dyn[[t]] 
plot.compliance.network(g,frame)
...
print(Delta.Network.vs.Per.plot)
print(Delta.Per.vs.Network.plot)
...
save.plots<-TRUE
if(save.plots){
  ggsave(c1.plot,file=paste(figures.dir ,"Model_","c1_plot",".pdf",sep=""), width=10, height=5)
  ggsave(w.plot,file=paste(figures.dir ,"Model_","w_plot",".pdf",sep=""),  width=10, height=5)
  ggsave(report.plot,file=paste(figures.dir ,"Model_","report_plot",".pdf",sep=""),  width=10, height=5)
  ggsave(per.audit.rate.plot,file=paste(figures.dir,"Model_",
                                        "per_audit_rate_plot",".pdf",sep=""),  width=10, height=5)
  ggsave(years.since.last.compliant.plot,file=paste(figures.dir,"Model_",
                            "years_since_last_compliant_plot",".pdf",sep=""),  width=10, height=5)
  ggsave(Delta.Network.vs.Per.plot,file=paste(figures.dir ,"Model_","Delta_Network_vs_Per_plot",
                                              ".pdf",sep=""),  width=10, height=5)
  ggsave(Delta.Per.vs.Network.plot,file=paste(figures.dir ,"Model_","Delta_Per_vs_Network_plot",
                                              ".pdf",sep=""),  width=10, height=5)
  ggsave(report.compliance.prob.plot,file=paste(figures.dir,"Model_",
                            "report_compliance_prob_plot",".pdf",sep=""),  width=10, height=5)
  ggsave(sample.report.trajectory.plot,
         file=paste(figures.dir,"Model_",
                    "sample_report_trajectory_plot",".pdf",sep=""),  width=15, height=10)
  ggsave(iterative.map.plot,
         file=paste(figures.dir,"Model_","iterative_map_plot",".pdf",sep=""),  width=10, height=5)
  ggsave(aggregated.dyn.plot,
