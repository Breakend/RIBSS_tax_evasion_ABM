...
for(t in 1:final.year){
  ...
  cV.new <- s.generation.discount*cV.past+as.numeric(past.year.compliant | past.year.penalized)
  cV.max <- s.generation.discount*cV.max+1
  ...
  #w <- propensity.prop.income.report*as.numeric(years.since.last.compliant<threshold.time) #*self.view.compliant
...
report.compliance.prob.plot <- ggplot(dat) +
  geom_point(aes(x=w, y=report, colour = recently.compliant,size=recently.penalized,alpha=c1.tilde))+
