...
media.feedback <- 0
media.mid.effect <- 0.40 ### at 40% tax gap media attention is at 50% of the saturation point. 
...
for(t in 1:final.year){
  ...
  if(media.effect){
    media.feedback <- beta.morale.media*tax.gap*cdf.lognormal(x=tax.gap, log(media.mid.effect),1/media.steepness)
  }
