#### Bomb Crater Effect
get.per.audit.bomb.crater<-function(q,n,simple.model=TRUE){
  if(simple.model){
    if(length(n)==1)
    {
      R<-0.58*q  ### (0.65-0.58)/2= 0.035 & 0.545=0.58-(0.65-0.58)/2
    } 
  }else{
    ### the bomb creater effect is called in the at the end of year n 
    ### but it effects perceptions for year n+1. So n+1 is really
    ### the years since last audit. Thus if you want the percieved audit at year 3 enter n=2.
    if(length(n)>0)
    {
      n<- n+1
      R<-min((0.035*n+0.545)*q,q)  ### (0.65-0.58)/2= 0.035  & 0.545=0.58-(0.65-0.58)/2
    } else
    {
      R <- Inf
    }
