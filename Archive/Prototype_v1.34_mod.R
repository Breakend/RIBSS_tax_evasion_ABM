...
for(t in 1:final.year){
  ...
  c1.tilde <- modified.c1.tilde.fn(c1,c2,tax.rate,per.audit.rate,per.penalty.rate*penalty.asymmetery.factor,
                          mu,nu,avg.nu,tax.complexity)
  ...
  #############################################################
  if(network.interactions){
    network.values <- get.network.interactions(Delta.Per,beta.morale.network,nn.int,prop.of.those.int)
  ...
  #############################################################
  ###   Compute the Media Effect on Tax Morale: Delta.Media           
  #############################################################
  if(media.effect){
    effective.beta.morale.media <- beta.morale.media*
      cdf.lognormal(x=tax.gap, log(media.mid.effect),1/media.steepness)
    Delta.Media <- 1-tax.gap
  }else{
    effective.beta.morale.media <- 0 
    Delta.Media <- 0
  }
  ...
  Delta.Morale <- (1-effective.beta.morale.network-effective.beta.morale.media)*Delta.Per + 
    effective.beta.morale.network*Delta.Network+
    effective.beta.morale.media*Delta.Media
