...
s.discount <- 0.7  ## half life is ~2 years
...
s.penalty.discount <- exp(-log(2)/30)  # half life is 30 years.

mu <- 0.07 #### this is the value for per.audit.rate*per.penalty.rate when c1.tilde is half way between c1 and c2. 


c2<-0.7
c1<- pmin(rexp(N, rate = log(2)/0.3),c2) ### median of the distribution is 0.3
c1<- sort(c1)
...
for(t in 1:final.year){
  ...
  ### Determine the personal willingness to comply 
  Delta <-fairness.fn(c1.tilde,c1,c2,tax.rate)
  
  ### Find Delta: those that were not compliant and not audited in the past year Delta=0 
  Delta <- Delta*as.numeric(past.year.compliant | past.year.audited.non.compliant)
  
  V.new <- personal.pro.compliance(V.past, Delta, s.discount)
  ...
  ### Note: willingness-to-comply includes audit perceptrions and network effects on 
  ...
  if(network.interactions){
    w<- sapply(tax.ids,FUN=function(i){
      if(length(nn[[i]])>0){
       r <- (1-beta.morale)*w[i]+beta.morale* mean(w[nn[[i]]])
      }else{
       r<- w[i]
      }
      return(r)
  ...
  random.numbers.to.compare[years.since.last.compliant>
                  threshold.time] <- 1 ## I don't like this but for now its ok. 
  ...
  report <- income
  report[!(compliant)] <- income[!(compliant)]*w[!(compliant)] 
  ...
  rownames(reporting.history) <- paste("% reported",seq((l-1),0,-1),"years ago",sep=" ")
  ...
  ### Letter
  letter <- (runif(N,0,1)<=letter.rate)
  
  ### Audit. 
  audit <- rep(FALSE,N)
  audit[letter] <- (runif(sum(letter),0,1)<=audit.rate.w.letter)
  audit[!letter] <-(runif(sum(!letter),0,1)<=audit.rate.wout.letter)
  ...
  ### Find those that recieve a letter and not compliant. Thus assume 100% efficency 
  ### in determining if non compliant when audited.
  letter.non.compliant <- letter & (!compliant)
  ...
  ### Correct penalized history by recording that an audited individual has paid
  ### his/her past K penalties.
  ...
  Q.new <- s.audit.discount*Q.past + as.numeric(audit)+ 
    impact.letter.wrt.audit*as.numeric(letter & !audit)
  ...
  V.new[penalized] <- personal.pro.compliance(V.new[penalized],
                                                          Delta.Penalty, s.discount)
...
c1.plot <- ggplot(dat) +
  geom_density(aes(x=c1, y = ..density..),alpha=0.5, fill="pink",color="red",size=0.1) +
  geom_density(aes(x=c1.tilde, y = ..density..),alpha=0.5, fill="lightblue",color="blue",size=0.1)+
  theme_bw() +
  xlab("tax rate") +
  ylab("density")+
...
w.plot <- ggplot(dat) +
  ...
  theme_bw() +
  xlab("propensity to comply as a %") +
  ylab("count")+
  ggtitle(paste("Histogram of propensity to comply",sep=""))  
