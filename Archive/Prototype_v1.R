...
mu <- 0.015 #### this is the value for per.audit.rate*per.penalty.rate when c1.tilde is half way between c1 and c2. 
...
for(t in 1:final.year){
  ...
  random.numbers.to.compare[years.since.last.compliant>
                  threshold.time] <- 1 ## PROBLEM: I don't like this but for now its ok. 
  ...
  ### Letter. For the time being we assume random letter
  ...
  ### Find those that recieve a letter and not compliant. 
  ...
  ### Correct penalized history by recording that an audited and penalized individual has paid
...
report.compliance.prob.plot <- ggplot(dat) +
  geom_point(aes(x=w, y=report, colour = c1.tilde),size=4)+
  xlab("prob comply (w)") +
