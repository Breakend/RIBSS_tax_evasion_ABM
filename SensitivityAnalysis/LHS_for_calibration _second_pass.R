...
model.parameters <- read.csv(paste(model.inputs.dir,"Exp.Design.model.parameters_second_pass",".csv",sep=""),
...
# lhs.tab <- apply(model.parameters[, c('Mode', 'Lower', 'Upper')], 1, function(x){
#   if(is.na(x[2]))
#     rep(x[1], 500)
#   else
#     runif(500, x[2], x[3])
...
#For calibration we don't need tax.rate changing.
cases$tax.rate.delta <- 0

##Making sure they add up to 1
cases$beta.media <- (1-cases$beta.personal)/2
cases$beta.network <- (1-cases$beta.personal)/2
...
write.csv(cases,file=paste(model.inputs.dir,"lhs.cases_calibration_second_pass",".csv",sep=""),row.names = FALSE)
