...
sens.dir <- "Outputs/Sensitivity/"

sensitivity.analysis <- function(random.seed.file = "Inputs/random.seed.table.csv",
                                 test.cases.file = "Inputs/lhs.cases_sensitivity.csv",
                                 start.model = 1,
                                 end.model = 2,
                                 start.case=2,
                                 end.case = -1,
                                 type = "PSOCK",
                                 outfile="sensitivity_output.csv", 
                                 logfile = "Outputs/Sensitivity/log.txt",
                                 parallelize.runs = F)
{
    ...
    for(seed in random.seed.table$random.seed)
    {
      if(seed.i == 0) {
        ...
        retval.0 <- tryCatch({
          ...
          tax.model(config = config.for.case.0, return.final.state.not.outputs = TRUE, cluster = cl, allow.tax.rate.changes = T)
        },
        error = function(e){
      ...
      retval.n <- tryCatch({
        #browser()
        tax.model(config = config.for.case.i, seed=seed, initial.state = final.state, cluster = cl, allow.tax.rate.changes = T)
      }, error = function(e){
      ...
      final.state.file.name <- paste0(sens.dir, "tax_model_", case.i, "_", seed.i, "_seed_value_", seed, ".csv")
...
res <- sensitivity.analysis(start.model=1, end.model=1, start.case = 1, end.case = 700,
                            outfile=paste0(sens.dir,"1-to-700.csv"),
                            logfile = paste0(sens.dir,"log_1-to-700.txt"), parallelize.runs = T)

res <- sensitivity.analysis(start.model=1, end.model=1, start.case = 701, end.case = 1400,
                            outfile=paste0(sens.dir,"701-to-1400.csv"),
                            logfile = paste0(sens.dir,"log_701-to-1400.txt"), parallelize.runs = T)

res <- sensitivity.analysis(start.model=1, end.model=1, start.case = 1401, end.case = 2000,
                            outfile=paste0(sens.dir,"1401-to-2000.csv"),
                            logfile = paste0(sens.dir,"log_1401-to-4000.txt"), parallelize.runs = T)
